...
/// api URL ///
const apiURL = 'https://song-lyrics-api.azharimm.dev';
...
async function searchSong(searchValue){
    const searchResult = await fetch(`${apiURL}/search?q=${searchValue}`)
...
function showData(data){
    ...
    <ul class="song-list">
      ${data.data
        .map(song=> `<li>
                    <div>
                        <strong>${song.artist}</strong> -${song.songTitle} 
                    </div>
                    <span data-lyrics="${song.songLyrics}" data-artist="${song.artist}" data-songtitle="${song.songTitle}"> get lyrics</span>
                </li>`
        )
        .join('')}
    </ul>
...
result.addEventListener('click', e=>{
    ...
    if (clickedElement.tagName === 'SPAN'){
        ...
        const songLyrics = clickedElement.getAttribute('data-lyrics')
        
        getLyrics(artist, songTitle, songLyrics)
    }
})

// Get lyrics for song
async function getLyrics(artist, songTitle, songLyrics) {
  
    const res = await fetch(songLyrics);
    
    const data = await res.json();
    console.log(data.data.songLyrics)
    const lyrics = data.data.songLyrics.replace(/(\r\n|\r|\n)/g, '<br>');
    result.innerHTML = ` 
    ...
    <p style="margin-top:20px; margin-bottom:100px">${lyrics}</p>
`    
